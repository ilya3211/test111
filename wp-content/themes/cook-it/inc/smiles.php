 ...
 */
add_filter( 'smilies_src', 'cook_it_smilies_src', 10, 3 );
function cook_it_smilies_src( $img_src, $img, $siteurl ) {
    return get_bloginfo('template_url') . '/assets/images/smilies/' . $img;
}
...
function comment_form_field_comment_add_smile( $comment_field ) {
    ...
    $dm_showsmiles = '';
    $dm_smiled = array();
	$is_show_comments_smiles = $wpshop_core->get_option( 'comments_smiles' );
	
    foreach ( $wpsmiliestrans as $tag => $dm_smile ) {
        if ( ! in_array($dm_smile,$dm_smiled) ) {
            $dm_smiled[] = $dm_smile;
            $tag = str_replace(' ', '', $tag);
            $dm_showsmiles .= '<img src="' . get_template_directory_uri() . '/assets/images/smilies/' . $dm_smile . '" alt="' . $tag . '"> ';
        }
    }
