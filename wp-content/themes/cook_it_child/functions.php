...
/**
 * Override cook_time_prepare function to fix "руб" bug
 * Original function incorrectly shows "руб" instead of time
 *
 * @param \WP_Post|null $post
 * @return string Cooking time in proper format
 */
if ( ! function_exists( 'cook_time_prepare' ) ) {
    function cook_time_prepare( $post = null ) {
        ...
        $total_time = (int) $recipe_prep_time + (int) $recipe_cook_time;

        if ( $total_time > 0 ) {
            // Return time in minutes with proper unit
            if ( $total_time < 60 ) {
                return $total_time . ' мин';
            } else {
